...
function tWithOffset(e, key, offset) {
  context.globalThis.lbDecrypted.HFNctz = {};
  return context.globalThis.lbDecrypted(e + offset - -309 - -46, key);
}

console.log('--- Brute-forcing Offset for Part 5 (e=710, key="LT*@") ---');
for (let offset = 150; offset < 200; offset++) {
  try {
    const val = tWithOffset(710, 'LT*@', offset);
