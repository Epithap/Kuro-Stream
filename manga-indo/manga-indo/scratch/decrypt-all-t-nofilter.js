...
const results = [];
for (let e = -500; e < 1500; e++) {
  for (const key of keys) {
    try {
      const val = t(e, key);
      // Let's be very lenient with matching: just printable characters
...
console.log('Total Decrypted Results:', results.length);
fs.writeFileSync('scratch/all-t-decrypted.json', JSON.stringify(results, null, 2));
