import fs from 'fs';
import vm from 'vm';

const code = fs.readFileSync('scratch/js_files/components-_yy6KL6v.js', 'utf8');

const rbStartStr = 'function Rb(){';
const rbEndStr = 'Rb=function(){return e},Rb()}';

const rbStartIdx = code.indexOf(rbStartStr);
const rbEndIdx = code.indexOf(rbEndStr, rbStartIdx);

const rbCode = code.substring(rbStartIdx, rbEndIdx + rbEndStr.length);

const lbStartStr = 'function Lb(e,t){';
const lbEndStr = 'Lb.HFNctz={},Lb.FwagKh=!0}let a=n[0],o=e+a,s=Lb.HFNctz[o];return s?r=s:(Lb.HNhjCk===void 0&&(Lb.HNhjCk=!0),r=Lb.lIkQXZ(r,t),Lb.HFNctz[o]=r),r}';

const lbStartIdx = code.indexOf(lbStartStr);
const lbEndIdx = code.indexOf(lbEndStr, lbStartIdx);

const lbCode = code.substring(lbStartIdx, lbEndIdx + lbEndStr.length);

const runner = `
${rbCode}
${lbCode}
globalThis.lbDecrypted = Lb;
`;

const context = vm.createContext({ globalThis: {} });
vm.runInContext(runner, context);

function t(e, key) {
  context.globalThis.lbDecrypted.HFNctz = {};
  return context.globalThis.lbDecrypted(e + 174 - -309 - -46, key);
}

function r(e, key) {
  context.globalThis.lbDecrypted.HFNctz = {};
  return context.globalThis.lbDecrypted(e - 336, key);
}

console.log('--- Decrypting s components and n.wwfvh ---');
try { console.log('r(918, "rFVS") =', r(918, 'rFVS')); } catch(e){}
try { console.log('t(542, "Zowf") =', t(542, 'Zowf')); } catch(e){}
try { console.log('t(608, "&EGx") =', t(608, '&EGx')); } catch(e){}
try { console.log('t(-5, "flu#") =', t(-5, 'flu#')); } catch(e){}
try { console.log('t(384, "&EGx") =', t(384, '&EGx')); } catch(e){}
try { console.log('t(-27, "0cSO") =', t(-27, '0cSO')); } catch(e){}
try { console.log('t(554, "l3mC") =', t(554, 'l3mC')); } catch(e){}
try { console.log('n.wwfvh decrypted =', '++ ' + t(542, 'Zowf') + t(608, '&EGx') + t(-5, 'flu#') + t(384, '&EGx') + t(-27, '0cSO') + t(554, 'l3mC')); } catch(e){}

// Let's decrypt t(563, '1JJx') + t(511, 'LT*@') (PDHOS key)
try { console.log('t(563, "1JJx") =', t(563, '1JJx')); } catch(e){}
try { console.log('t(511, "LT*@") =', t(511, 'LT*@')); } catch(e){}
try { console.log('n.PDHOS decrypted =', t(563, '1JJx') + t(511, 'LT*@') + 's-key'); } catch(e){}
